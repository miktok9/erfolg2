...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🏛️ Die Geschichte der Frauen in der Antike!\n\n#FrauenGeschichte #AntikeGeschichte #Geschichte #Bildung"

        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🏛️ Neues Video!"
        ...
        video = upload.video(
            ...
            name=title or 'Die Geschichte der Frauen in der Antike',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Die Geschichte der Frauen in der Antike! 🏛️"
    title = sys.argv[3] if len(sys.argv) > 3 else "Die Geschichte der Frauen in der Antike"
